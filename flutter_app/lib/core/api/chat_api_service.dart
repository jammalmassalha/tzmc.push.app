...
class ChatApiService {
  ...
  Future<AccreditationAgentResponse> askAccreditationAgent(String question) async {
    final response = await _client.post<Map<String, dynamic>>(
      ...
      options: Options(
        receiveTimeout: NetworkTimeouts.aiAgentTimeout,
        sendTimeout: NetworkTimeouts.aiAgentTimeout,
      ),
      retryOptions: const RetryOptions(retries: 1, timeout: NetworkTimeouts.aiAgentTimeout),
    );
    if (!response.isSuccessful) {
      throw ApiException(
