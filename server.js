...
async function processReplyPayload(rawPayload = {}, resolvedUser = '') {
    ...
    try {
        ...
        if (subRows && subRows.length > 0) {
            ...
            const isUserRestricted = status !== '1' && exceptionStatus !== '1';
            if (isUserRestricted) {
                isRestricted = true;
...
function ensureRegistrationFlowOnly(req, requestedUser) {
    ...
    if (sessionUser === normalizedRequestedUser) {
        return { ok: false, status: 403, message: 'Authenticated users cannot request registration SMS code' };
    }
    return {
        ok: false,
        status: 403,
    ...
    async (req, res) => {
        ...
        try {
            ...
            mysqlLogsService.pool.query(
                'UPDATE `Subscribe` SET `ExeptionName` = ? WHERE `User` = ?',
                [collectedData.name, user]
            ).catch(err => console.error('[SECRETARY-BOT] Failed to update ExeptionName in DB:', err && err.message ? err.message : err));
            fetchWithRetry(GOOGLE_SHEET_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            }, { timeoutMs: 10000, retries: 2 }).catch(err => console.error('[SECRETARY-BOT] Failed to update ExeptionName in sheet:', err && err.message ? err.message : err));
