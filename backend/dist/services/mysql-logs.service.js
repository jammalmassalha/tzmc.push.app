 ...
 */
function normalizeLifecycleDateTimeForStorage(value, fallback) {
    const timestamp = parseFlexibleTimestamp(value);
    if (!timestamp || !Number.isFinite(timestamp)) {
        return fallback;
    }
...
class MysqlLogsService {
     ...
     */
    async ensureLifecycleTimestampColumns() {
        if (this.lifecycleTimestampColumnsReady)
            return;
        ...
        try {
            ...
            await this.pool.execute(`UPDATE \`${this.tableName}\` SET \`sentDateTime\` = \`DateTime\` WHERE \`sentDateTime\` IS NULL`);
        }
        catch (err) {
            console.warn('[MYSQL] sentDateTime backfill warning:', String(err.message || ''));
        }
        try {
            // Index for optimized chronological message retrieval per chat partner.
            await this.pool.execute(`CREATE INDEX \`idx_chat_sentDateTime\` ON \`${this.tableName}\` (\`From\`, \`sentDateTime\`)`);
        }
        catch (err) {
            const code = err.code;
            const message = String(err.message || '');
     ...
     */
    async ensureMessageActivitiesLifecycleColumns() {
        if (this.messageActivitiesLifecycleReady)
            return;
        const columnStatements = [
        ...
        try {
            ...
            await this.pool.execute(`UPDATE \`MessageActivities\`
         SET \`sentDateTime\` = COALESCE(FROM_UNIXTIME(\`ActionTimestamp\` / 1000), \`CreatedAt\`, NOW(3))
         WHERE \`sentDateTime\` IS NULL`);
        }
        catch (err) {
            console.warn('[MYSQL] MessageActivities sentDateTime backfill warning:', String(err.message || ''));
        }
        ...
        try {
            const [indexRows] = await this.pool.query(`SHOW INDEX FROM \`MessageActivities\` WHERE \`Column_name\` = 'MessageId'`);
        ...
        if (!hasMessageIdIndex) {
            ...
            indexStatements.unshift(`CREATE INDEX \`idx_msg_messageid\` ON \`MessageActivities\` (\`MessageId\`(100))`);
        }
     ...
     */
    async markActivitiesRead(reader, chatId) {
        await this.ensureMessageActivitiesTable();
        const safeReader = toTrimmedString(reader).toLowerCase();
        const safeChatId = toTrimmedString(chatId).toLowerCase();
        if (!safeReader || !safeChatId)
            return 0;
        try {
          ...
          AND (\`Sender\` = ? OR \`Recipient\` = ? OR \`Recipient\` LIKE ?`;
            ...
            return typedRows.map((row) => {
                ...
                if (actionType === 'edit-action') {
                    ...
                    if (receiveDateTime > 0)
                        record.receiveDateTime = receiveDateTime;
                    const readDateTime = parseFlexibleTimestamp(row.readDateTime);
                    if (readDateTime > 0)
                        record.readDateTime = readDateTime;
                    if (groupId && sender) {
    ...
    async ensureSubscribeTable() {
      ...
      `);
        ...
        `);
            ...
            try {
                const [keys] = await this.pool.query('SHOW KEYS FROM `Subscribe` WHERE Key_name = "PRIMARY"');
                const hasPrimaryKey = keys && keys.length > 0;
                let isUserPk = false;
                if (hasPrimaryKey) {
                    isUserPk = keys.some((k) => String(k.Column_name || k.column_name || '').toLowerCase() === 'user');
                }
                if (!isUserPk) {
            ...
            try {
                const [columns] = await this.pool.query('SHOW COLUMNS FROM `Subscribe`');
                const existingCols = new Set(columns.map((c) => String(c.Field || c.field || '').toLowerCase()));
