...
export function normalizeIsraeliPhoneNumberToLocal(value: unknown): string {
  const rawDigits = digitsOnly(value);
  if (!rawDigits) return '';
  if (/^0\d{9}$/.test(rawDigits)) return rawDigits;
  if (/^5\d{8}$/.test(rawDigits)) return `0${rawDigits}`;
  ...
  if (!/^5\d{8}$/.test(localDigits)) return '';
  return `0${localDigits}`;
}

export function toIsraeliInternationalPhoneDigits(value: unknown): string {
...
export function maskPhoneForLogs(value: unknown): string {
  ...
  if (!candidate) return '';
  if (candidate.length <= 4) return '*'.repeat(candidate.length);
  return `***${candidate.slice(-4)}`;
}

export function formatAuthCodeSmsMessage(template: unknown, code: unknown): string {
...
export function buildInforuSmsXmlPayload({
  username,
  apiToken,
  message,
  phone,
...
export function extractInforuStatusCode(rawResponse: unknown): string {
  const match = String(rawResponse || '').match(/<Status>\s*([^<\s]+)\s*<\/Status>/i);
  return match ? String(match[1] || '').trim() : '';
}

export function extractInforuStatusDescription(rawResponse: unknown): string {
