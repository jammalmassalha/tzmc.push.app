...
/**
 * Live status of a user row in the Google Sheet `Subscribe` tab.
 * Returned by an optional provider so that {@link MysqlLogsService.checkAuth}
 * can treat the sheet as the source of truth for the restricted flag instead of
 * the periodically-synced MySQL row (Option A).
 *
 * - Resolve to `{ found: false }` when the user does not exist in the sheet.
 * - Resolve to `{ found: true, status, exceptionStatus, fullName }` with the raw
 *   Col G (`status`) and Col H (`exceptionStatus`) values when the user exists.
 * - Resolve to `null` (or reject) when the live value cannot be determined, so
 *   the caller falls back to the MySQL row.
 */
export interface SubscribeLiveStatus {
  found: boolean;
  status?: string;
...
export type SubscribeStatusProvider = (username: string) => Promise<SubscribeLiveStatus | null>;
...
export interface MysqlLogInsertPayload {
  ...
  fileUrl?: string;
  /**
   * Human-readable display name of the actual user who sent a group message.
   * For groups the `sender` column stores the group ID, not the user, so this
   * column preserves the sender label that is shown in the FCM notification
...
export interface FlutterPushRegistrationDebugEvent {
  action: string;
...
export class MysqlLogsService {
  ...
  private subscribeStatusProvider: SubscribeStatusProvider | null = null;
   ...
   */
  private async ensureGroupSenderNameColumn(): Promise<void> {
    if (this.groupSenderNameColumnReady) return;
    try {
      await this.pool.execute(
        `ALTER TABLE \`${this.tableName}\` ADD COLUMN \`GroupSenderName\` VARCHAR(255) NULL DEFAULT NULL`
      );
    } catch (err: unknown) {
      const code = (err as { code?: string }).code;
      const message = String((err as { message?: string }).message || '');
   ...
   */
  async hasDuplicateLog(sender: string, recipient: string, dateTime: unknown): Promise<boolean> {
    await this.ensureDedupIndex();
  ...
  private async ensureFlutterPushRegistrationDebugTable(): Promise<void> {
    if (this.flutterPushRegistrationDebugTableReady) return;
    try {
      await this.pool.execute(`
  ...
  async adminListCommunityGroups(): Promise<CommunityGroupDbConfig[]> {
    await this.ensureCommunityGroupsTables();
    try {
      const [groupRows] = await this.pool.execute<RowDataPacket[]>(
        'SELECT `GroupId`, `GroupName`, `IsEnabled` FROM `CommunityGroups` ORDER BY `GroupId`'
      );
      if (!groupRows.length) return [];
  ...
  async upsertCommunityGroup(config: CommunityGroupDbConfig): Promise<boolean> {
    await this.ensureCommunityGroupsTables();
    const conn = await this.pool.getConnection();
    try {
      await conn.beginTransaction();
      ...
      await conn.execute(
        `INSERT INTO \`CommunityGroups\` (\`GroupId\`, \`GroupName\`, \`IsEnabled\`)
         VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE
           \`GroupName\` = VALUES(\`GroupName\`),
           \`IsEnabled\` = VALUES(\`IsEnabled\`)`,
        [config.groupId, config.groupName, config.isEnabled !== false ? 1 : 0]
      );
      ...
      if (validMembers.length > 0) {
        ...
        await conn.execute(
          `INSERT IGNORE INTO \`CommunityGroupMembers\` (\`GroupId\`, \`Phone\`) VALUES ${ph}`,
          validMembers.flatMap((p) => [config.groupId, p])
        );
      }
      ...
      if (validWriters.length > 0) {
        ...
        await conn.execute(
          `INSERT IGNORE INTO \`CommunityGroupWriters\` (\`GroupId\`, \`Phone\`) VALUES ${ph}`,
          validWriters.flatMap((p) => [config.groupId, p])
        );
      }
  ...
  async deleteCommunityGroup(groupId: string): Promise<boolean> {
    await this.ensureCommunityGroupsTables();
    try {
      const [result] = await this.pool.execute<import('mysql2').ResultSetHeader>(
        'DELETE FROM `CommunityGroups` WHERE `GroupId` = ?',
        [groupId]
      );
  ...
  async setCommunityGroupEnabled(groupId: string, enabled: boolean): Promise<boolean> {
    await this.ensureCommunityGroupsTables();
    try {
      const [result] = await this.pool.execute<import('mysql2').ResultSetHeader>(
        'UPDATE `CommunityGroups` SET `IsEnabled` = ? WHERE `GroupId` = ?',
        [enabled ? 1 : 0, groupId]
      );
      const affected = (result as { affectedRows?: number }).affectedRows ?? 0;
  ...
  async ensureMessageActivitiesTable(): Promise<void> {
    if (this.messageActivitiesTableReady) return;
    try {
      await this.pool.execute(`
   ...
   */
  setSubscribeStatusProvider(provider: SubscribeStatusProvider | null): void {
   ...
   */
  private buildAuthResult(rawStatus: unknown, rawExceptionStatus: unknown, rawFullName: unknown): any {
...
// ---------------------------------------------------------------------------
// LicenserService — checks the employment DB to detect pending activations
// ---------------------------------------------------------------------------
...
export interface LicenserConfig {
  ...
  password: string;
  database: string;
  /** Table name that holds employee records, e.g. "employ" */
  table: string;
  /** Column that stores the employee phone number, e.g. "Phone" */
  phoneColumn: string;
  connectionLimit: number;
}
 ...
 */
export class LicenserService {
  private readonly pool: Pool;
  private readonly table: string;
  private readonly phoneColumn: string;
   ...
   */
  async checkPhoneInEmployTable(phone: string): Promise<boolean> {
    const normalized = String(phone || '').trim();
    if (!normalized) return false;
    const [rows] = await this.pool.query<RowDataPacket[]>(
      `SELECT 1 FROM \`${this.table}\` WHERE \`${this.phoneColumn}\` = ? LIMIT 1`,
      [normalized]
    );
    return Array.isArray(rows) && rows.length > 0;
  }
...
export function createLicenserServiceFromEnv(env: NodeJS.ProcessEnv = process.env): LicenserService | null {
  ...
  const connectionLimit = Math.max(
    1,
    Math.min(toPositiveInteger(env.LICENSER_DB_CONNECTION_LIMIT, 3), 10)
  );
