...
export function createMysqlLogsServiceFromEnv(env: NodeJS.ProcessEnv = process.env): MysqlLogsService {
  ...
  if (!user) missing.push('LOGS_DB_USER');
  if (!password) missing.push('LOGS_DB_PASSWORD');
  if (!database) missing.push('LOGS_DB_NAME');
  if (missing.length > 0) {
    throw new Error(
      `[MYSQL] Missing required database environment variable(s): ${missing.join(', ')}. ` +
