...
export interface SendPushOptions {
  ...
  limitPerUserEndpoints?: boolean;
  maxPerUserEndpoints?: number;
  messageId?: string;
  skipBadge?: boolean;
  dedupLog?: boolean;
}
...
export interface NotificationServiceConfig {
  ...
  maxPushTextLength?: number;
  defaultMaxEndpointsPerUser?: number;
  singleTargetMaxSafeSubscriptions?: number;
  unknownUserFallbackMaxEndpoints?: number;
}

/** Minimal interface the service needs from the host environment. */
export interface NotificationServiceDeps {
  /** web-push sendNotification */
  sendNotification: (sub: PushSubscription, payload: string, options: unknown) => Promise<void>;
  /** Resolve the canonical phone/key for a user identifier */
  normalizeUserKey: (value: unknown) => string;
  /** Resolve subscription device type */
  normalizeSubscriptionType: (value: unknown) => string;
  /** Deduplicate subscriptions by endpoint */
  dedupeSubscriptionsByEndpoint: (subs: PushSubscription[]) => PushSubscription[];
  /** Collect usernames from input */
  parseUsernamesInput: (value: unknown) => string[];
  /** Build alias-to-canonical user lookup map */
  buildUserAliasLookupMap: (users: string[]) => Map<string, string>;
  /** Resolve canonical user from alias map */
  resolveCanonicalUserFromLookup: (key: string, aliases: Map<string, string>) => string;
  /** Generate a unique message id */
  generateMessageId: () => string;

  /** Fetch subscriptions from sheet/cache */
  getSubscriptionFromSheet: (users: string[], options?: { forceRefresh?: boolean }) => Promise<PushSubscription[]>;
  /** Discover all subscriptions for auth refresh */
  getAllSubscriptionsForAuthRefresh: (options: { usernames: string[] }) => Promise<{ subscriptions: PushSubscription[] }>;
  /** Get local device subscriptions for users */
  getLocalDeviceSubscriptionsForUsers: (users: string[]) => PushSubscription[];
  /** Remove a local device subscription endpoint */
  removeLocalDeviceSubscriptionEndpoint: (endpoint: string) => boolean;
  /** Remove stale subscriptions from remote sheet */
  removeStaleSubscriptionsFromSheet: (endpoints: string[]) => Promise<{ clearedSubscriptions: number } | null>;
  /** Build subscription cache key */
  buildSubscriptionCacheKey: (users: string[]) => string;
  /** Prune an endpoint from cache */
  pruneSubscriptionCacheEndpoint: (endpoint: string) => void;
  /** Subscription cache (Map<string, { at: number; subscriptions: PushSubscription[] }>) */
  subscriptionCache: Map<string, { at: number; subscriptions: PushSubscription[] }>;

  /** Read/write the in-memory unread counts map */
  unreadCounts: Record<string, number>;
  /** Trigger a debounced state save */
  scheduleStateSave: () => void;
  /** Log a push notification status to MySQL */
  logNotificationStatus: (...args: unknown[]) => void;
  /** Build mobile auth JSON string for logs */
  buildMobileSubscriptionAuthJsonForLog: (target: string, subs: PushSubscription[]) => string;

  /** Auth-refresh push type constant */
  AUTH_REFRESH_PUSH_TYPE: string;
}

// ─── Service ────────────────────────────────────────────────────────────────

export class NotificationService {
  private readonly maxPushPayloadBytes: number;
  private readonly maxPushTextLength: number;
  private readonly defaultMaxEndpointsPerUser: number;
  private readonly singleTargetMaxSafe: number;
  private readonly unknownUserFallbackMax: number;
  private readonly deps: NotificationServiceDeps;

  constructor(config: NotificationServiceConfig, deps: NotificationServiceDeps) {
  ...
  trimPushTextValue(value: unknown, maxLength = this.maxPushTextLength): string {
  ...
  buildCompactPushCustomData(rawData: Record<string, unknown> = {}, messageType = ''): Record<string, unknown> {
    if (!rawData || typeof rawData !== 'object') return {};
    const compact: Record<string, unknown> = {};
    Object.entries(rawData).forEach(([key, rawValue]) => {
      if (rawValue === undefined || rawValue === null) return;
      if (key === 'membersToNotify') return;
      if (key === 'groupMembers') {
  ...
  buildPushPayloadString(payloadData: Record<string, unknown> = {}, options: PushPayloadOptions = {}): string {
    ...
    let compactData: Record<string, unknown> = { ...payloadData };
    let payload = JSON.stringify(buildPayloadEnvelope(compactData));
    if (Buffer.byteLength(payload, 'utf8') <= this.maxPushPayloadBytes) return payload;
    ...
    const emergencyData: Record<string, unknown> = {
      ...
      messageText: this.trimPushTextValue(
        compactData.messageText || compactData.groupMessageText || compactData.body || '',
        500
      ),
  ...
  isLikelyPhoneUserKey(userKey: unknown): boolean {
  ...
  limitSubscriptionsPerUser(
    subscriptions: PushSubscription[] = [],
    maxPerUser = this.defaultMaxEndpointsPerUser
  ): PushSubscription[] {
    const normalizedMax = Math.max(1, Number(maxPerUser) || this.defaultMaxEndpointsPerUser);
    const byUser = new Map<string, { mobile: PushSubscription[]; pc: PushSubscription[]; unknown: PushSubscription[] }>();
    ...
    (Array.isArray(subscriptions) ? subscriptions : []).forEach((subscription) => {
      ...
      if (type === 'mobile') { bucket.mobile.push(subscription); return; }
      if (type === 'pc') { bucket.pc.push(subscription); return; }
      bucket.unknown.push(subscription);
    });

    const selected: PushSubscription[] = [];
    orderedUsers.forEach((userKey) => {
      const bucket = byUser.get(userKey);
      if (!bucket) return;
      const pickedForUser: PushSubscription[] = [];
      const pickOne = (list: PushSubscription[]): void => {
        if (!Array.isArray(list) || !list.length) return;
        pickedForUser.push(list[0]);
      };
      pickOne(bucket.mobile);
      pickOne(bucket.pc);
      if (!pickedForUser.length) pickOne(bucket.unknown);
  ...
  async sendPushNotificationToUser(
    ...
    // 1. Prepare content
    const msgBody = (message.body && typeof message.body === 'object' ? message.body : {}) as Record<string, unknown>;
    const customData = (message.data && typeof message.data === 'object' ? message.data : {}) as Record<string, unknown>;
    const messageType = String(customData.type || '').trim().toLowerCase();
    const isGroupScopedPush = Boolean(
      customData.groupId || customData.groupName || messageType === 'group-update' || messageType === 'reaction' ||
      (Array.isArray(customData.groupMembers) && (customData.groupMembers as unknown[]).length)
    );
    const imageUrl = (message.image as string) || null;
    const fileUrl = (customData.fileUrl as string) || null;
    ...
    let msgTitle = (message.title as string) || 'Work Alert';
    let msgText = (msgBody.shortText as string)
      || (msgBody.longText as string)
    ...
    const messageId = options.messageId || (message.messageId as string) || this.deps.generateMessageId();
    ...
    let normalizedTargetUsers = Array.from(
      new Set(targetUsersArray.map((u) => this.deps.normalizeUserKey(u)).filter(Boolean))
    );
    ...
    const normalizeAndFilterTargetSubscriptions = (
      subscriptions: PushSubscription[],
      filterOptions: { allowUnknownUser?: boolean } = {}
    ): PushSubscription[] => {
      ...
      const matched: PushSubscription[] = [];
      const unknownWithoutUser: PushSubscription[] = [];
      normalized.forEach((subscription) => {
        ...
        if (rawUser) return;
        if (!allowUnknownUser || !singleTargetUser) return;
    ...
    let rawSubscriptions = normalizeAndFilterTargetSubscriptions(
      await this.deps.getSubscriptionFromSheet(targetUsersArray),
      { allowUnknownUser: true }
    );
    if (!rawSubscriptions.length) {
      rawSubscriptions = normalizeAndFilterTargetSubscriptions(
        await this.deps.getSubscriptionFromSheet(targetUsersArray, { forceRefresh: true }),
        { allowUnknownUser: true }
      );
    }
    if (!rawSubscriptions.length) {
    ...
    if (localSubscriptions.length) {
      rawSubscriptions = normalizeAndFilterTargetSubscriptions(
        [...rawSubscriptions, ...localSubscriptions],
        { allowUnknownUser: true }
      );
      const cacheKey = this.deps.buildSubscriptionCacheKey(targetUsersArray);
    ...
    const recipientAuthJsonForLog = this.deps.buildMobileSubscriptionAuthJsonForLog(
    ...
    const sendToSubscriptions = async (
      subscriptions: PushSubscription[],
      allowBadgeIncrement: boolean
    ): Promise<Array<{ ok: boolean; username: string; badge?: number; statusCode?: string | number; message?: string; endpoint?: string }>> => {
      ...
      return Promise.all(
        subscriptions.map(async (subscription) => {
          ...
          if (shouldIncrementBadge && resolvedUserKey) payloadData.badgeCount = currentCount;
          ...
          try {
            ...
            return { ok: true, username: (subscription.username as string) || resolvedUserKey || 'unknown', badge: currentCount, endpoint: subscription.endpoint };
          } catch (err: unknown) {
            ...
            return { ok: false, username: (subscription.username as string) || resolvedUserKey || 'unknown', statusCode, message: errorObj.message, endpoint: subscription.endpoint };
          }
        })
      );
    };
    ...
    if (singlePerUser) {
      const onePerUser = new Map<string, PushSubscription>();
    ...
    const appendResultsToLogs = (results: Array<{ ok: boolean; username: string; badge?: number; statusCode?: string | number; message?: string }>): void => {
    ...
    if (successCount === 0 && allowSecondAttempt) {
      const cacheKey = this.deps.buildSubscriptionCacheKey(targetUsersArray);
      if (cacheKey) this.deps.subscriptionCache.delete(cacheKey);
    ...
    const staleEndpoints = Array.from(new Set(
      sendResults
    ...
    if (staleEndpoints.length) {
      ...
      staleEndpoints.forEach((endpoint) => {
        if (this.deps.removeLocalDeviceSubscriptionEndpoint(endpoint)) localRemoved += 1;
      });
      let staleCleanupSummary: { clearedSubscriptions: number } | null = null;
      try {
